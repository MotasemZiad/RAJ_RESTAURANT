...
import 'dart:io';
import 'package:raj_resturant/app/data/network_helper/strings.dart';
...
class DioHelper {
  ...
  static late final Dio _raj_resturantDio;
  ...
  static void init() {
    ...
    _raj_resturantDio = Dio(
      BaseOptions(
        baseUrl: Links.raj_resturantBaseLink,
        receiveDataWhenStatusError: true,
      ),
    );
    _dio.interceptors.add(dioLoggerInterceptor);
    _raj_resturantDio.interceptors.add(dioLoggerInterceptor);
  }
  ...
  static get(String url,
      ...
      Function? onLoading,
      bool israj_resturantLink = false}) async {
    try {
      // 1) indicate loading state
      onLoading?.call();
      // 2) try to perform http request
      ...
      if (israj_resturantLink == false) {
        ...
        response = await _raj_resturantDio
            .get(
              url,
              onReceiveProgress: onReceiveProgress,
              queryParameters: queryParameters,
